...
class SubwordComplex(UniqueRepresentation, SimplicialComplex):
    ...
    def brick_fan(self):
        ...
        It is the (outer/inner) normal fan of the brick polyhedron with (positive/negative) sign of ``self``.
        Its chambers are given by the union of images of the fundamental chamber for elements ``w`` in
        ``F.kappa_preimage()`` for ``F`` in ``self``.

        .. SEEALSO::

            :func:`kappa_preimage() <sage.combinat.subword_complex.SubwordComplexFacet.kappa_preimage>`
        ...
        EXAMPLES::
            ...
            Rational polyhedral fan in 2-d lattice N
        """
        weights = self.group().fundamental_weights()
        cones = []
        for F in self:
            rays = []
            for w in F.kappa_preimage():
                rays += [w * v for v in weights]
            if not rays == []:
